...
{	int l, c;
	...
	{	for(c=0; c<colunas; c++)
		{	
            if(l <= maxValor)
			{
				original[l][c] = (int)(original[l][c] * percent);
			}
			else if(l >= (linhas - 1) - maxValor)
			{
				original[l][c] = (int)(original[l][c] * percent);
			}
			else if((l > maxValor && (l < (linhas - 1) - maxValor) && c <= maxValor || c >= (colunas - 1) - maxValor))
			{
				original[l][c] = (int)(original[l][c] * percent);
			}
		}
	...
	{	printf("\n\nMini-photoshop\n\n");
		...
		{	case '1':
			...
			case '8':
				...
				printf("\nInsira a largura da moldura (qtde de pixels para o efeito de borda): ");
				scanf("%i", &borda);
				moldura(mat, linhas, colunas, borda);
				break;
		}
	}
	return 0;
